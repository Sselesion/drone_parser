...
wb = load_workbook("./get_excel/template.xlsx")
    ...
    for key_component in dataset.keys():
        ...
        for elem in item.keys():
            for key in item[elem]:
                ...
                ws.cell(current_row, column_write).alignment = Alignment(
                    horizontal="center"
                )
                ws.cell(current_row, column_write).font = Font(size=11, name="Arial")
                ws.cell(current_row, column_write).value = item[elem][key] if item[elem][key] else '-'
            current_row += 1
    wb.save("result.xlsx")
